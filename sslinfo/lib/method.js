...
module.exports = {
  ...
  getSSLResults: function(hostData) {
    var tasks = _.values(methods).map(function (d) {
      let options = _.extend({ protocol: d }, hostData)
   ...
   * @param options {{ host: string, port: number, timeOutMs: number, protocol: { name: string, commonName: string } }}
   * @returns {promise}
   */
